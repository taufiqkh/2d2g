...
FlatPixelProjection.prototype.pixelDistanceToLngDegrees = function(pixels) {
    var origin = this.fromLatLngToPoint(new google.maps.LatLng(0, -this.offsetLng));
    origin.x += pixels;
    return this.fromPointToLatLng(origin).lng() + this.offsetLng;
}

FlatPixelProjection.prototype.addLatPixels = function(latLng, pixels) {
    var point = this.fromLatLngToPoint(latLng);
    point.y += pixels;
    return this.fromPointToLatLng(point);
}